...
public class Fireworks implements Runnable {
	...
	private boolean sound = true; // play an "explosion" sound for each star
									// burst
	private int explosions = 5; // the number of explosions (0 ==> infinite)
	...
	/**
	 * Starts the fireworks display. This method starts a new thread, making it
	...
	/**
	 * Launches the fireworks at random intervals uniformly distributed in [0 ..
	 * maxDelay-1].
	 */
	...
	/**
	 * Draws a "star burst" explosion consisting of lines radiating from a
	 * central point.
	 * 
	 * @param x
	...
	public class AnimatedLine implements Runnable {
		...
		public AnimatedLine(int xStart, int yStart, int xEnd, int yEnd, int step, Color color, boolean burst) {
			this.step = step;
			this.color = color;
			this.step = step;
			this.burst = burst;
		 ...
		 * continues until it reaches "length" in length.
		 * 
		 * @param xStart
		 *            The X-coordinate of the start of the line segment.
		 * @param yStart
		...
		public AnimatedLine(int xStart, int yStart, int length, double theta, int step, Color color, boolean burst) {
			this.xStart = xStart;
			this.yStart = yStart;
			this.theta = theta;
			this.length = length;
